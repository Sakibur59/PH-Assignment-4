...
2. for create element: creating element using document.createElement("div"). and insert to their parent element using appendChild()
3. Event bubbling is an event starts from the target element and then propagates through its parent element(inluding the way of root)
...
4. Event delegation is a technique where we use in parent element instead of adding separate event to multiple child.
  why it's useful:
...
5. preventDefault()-> it's stop the defalut browser's action of an event.
